let a : string = 'wellcome to function type apply';
console.log(a);
...
let test1Let = function(x:string) : number{
    console.log('함수표현식')
    return 1
}
...
//잘보고 활용하쟈...
function h2(x:number|string) : number {
        if(typeof(x) == 'string'){
...
function 장가가자(돈:number , 집:boolean, 매력:매력점수):string|void 
{
    let score :number = 0;
    let 집점수 : number = 0;
    let 매력포인트 : number = 0;
